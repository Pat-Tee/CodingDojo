...
class SLL{
    ...
    removeFromBack() {
        ...
        while (pointer.next.next != null) { //traverse to the next-to-last node
            pointer = pointer.next;
        }    
        pointer.next = null;                //next-to-last node's next value made null, so next-to-last node is now the last node in the list
        }
    }
}
