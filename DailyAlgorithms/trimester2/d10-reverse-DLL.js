...
class DLL {
    ...
    reverseList() {
        ...
        if (this.head == null || this.head == this.tail)//if empty list or list of one, return
            return this;

        let pointer1 = this.head;                       // store head in a pointer
        let pointer2 = pointer1.next;                   // store head.next in a pointer
        
        let temp = this.tail;                           // temp stores tail
        this.tail = this.head;                          // swap the head and the tail
        this.head = temp;                               //

        for (let i=0; i < this.length-1; i++) {         // use length as limit for loop
            //console.log("for loop i = "+i);             // debug
            pointer1 = pointer2;                        // pointer2 maintains our .next in-original-list
            pointer2 = pointer2.next;                   // advance pointer2

            temp = pointer1.prev;                       // using temp swap the node's .prev and .next
            pointer1.prev = pointer1.next;              //
            pointer1.next = temp;                       //
        }
        return this;
    }
