...
class User:
    ...
    def valid_login(user):
        length = len(user['login_name'])
        if ( length < 3 or length > 16 ):
            print('Login name is outside length range check')
            flash("Login name must be at least 3 characters and no more than 16 characters long.")
        return True
    ...
    def get_all(cls):
        ...
        results = connectToMySQL('users_cars__schema').query_db(query)
    ...
    @classmethod
    def save(cls, data):
        query = "INSERT INTO users ( first_name, last_name , email, password, created_at, updated_at, login_name ) VALUES ( %(first_name)s , %(last_name)s, %(email)s , %(password)s, NOW() , NOW(), %(login_name)s );"
    ...
    def check_login(cls, data):
        query = "SELECT * FROM users WHERE login_name = %(login_name)s"
